...
namespace PracticaFirstTask
{
	...
	public class Person
	{
		...
		public Person(int id, string surname, string name, string patronymic, string passport, string numPhone, string mail)
		{
			...
			if (numPhone[0] == '+' && numPhone[0] == '7' && numPhone.Length == 12)
			{
				this.numPhone = numPhone;
			}
			else if (numPhone[0] == '8' && numPhone.Length == 11)
			{
				this.numPhone = numPhone;
			}
			else
		...
		public bool passpor(string passport)
        {
			for (int i = 0; i < passport.Length; i++) {
				return (int)passport[i] >= 48 && (int)passport[i] <= 57;
			}
			return false;
        }
		...
		public bool IsValidEmail(string email)
		{
			...
			while (email[i] != '@')
			{
				if ((int)email[i] >= 97 && (int)email[i] <= 122 || (int)email[i] == 95)
				{
					sas = true;
				}
				else
